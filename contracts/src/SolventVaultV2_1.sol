...
import {ILendingViews} from "./interfaces/ILendingViews.sol";

/// @notice ERC-4626 vault for the Solvent depeg-guardian product. Identical to
/// `SolventVaultV2` EXCEPT for an INIT-aware `totalAssets()`: it additionally
/// values an open bridge (INIT lending) position so the share price is
/// preserved across a `BRIDGE_VIA_LENDING` action (which moves the risk asset
/// into a lending position and pulls out borrowed safe asset). Shares
/// (`svUSDT0`) and the whole agent/owner surface are unchanged vs V2 — kept
/// verbatim to minimise churn.
contract SolventVaultV2_1 is ERC4626, ReentrancyGuard {
    ...
    /// (decimal-aware, nominal 1:1) + the value of any open bridge (INIT lending)
    /// position: net of collateral (in the asset's own units) minus debt
    /// (normalised from safe-asset units to asset units, exactly like the safe
    /// balance). This is what makes the share price survive a `BRIDGE_VIA_LENDING`:
    /// the risk asset that left the vault re-appears as `collateralUnderlying()`,
    /// the borrowed safe asset is counted both as a vault balance AND offset by
    /// `debtUnderlying()`, so the two cancel.
    function totalAssets() public view override returns (uint256) {
        ...
        if (policy.bridgeVenue != address(0)) {
            ...
            // collateral is in the asset's OWN units (USDY 18dec) -> add directly.
            coll = v.collateralUnderlying();
            // debt is in safe-asset units (USDC 6dec) -> normalise to asset units.
            debtInAsset = (v.debtUnderlying() * (10 ** ad)) / (10 ** sd);
        }
